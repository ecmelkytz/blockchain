...
impl Blockchain {
  ...
  pub fn approve_and_send(&mut self, transaction: Transaction, sign: Signature, public_key: PublicKey ) {
    let secp = Secp256k1::new();
    if secp.verify(&transaction.calculate_hash(), &sign, &public_key).is_ok() {
